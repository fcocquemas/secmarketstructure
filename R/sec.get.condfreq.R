#' Get conditional trade and cancelation frequency data from 
#' the SEC market structure website.
#'
#' By default, tries to retrieve all available dates (which is
#'   around 11 MB for conditional distribution).
#' 
...
#'   dt <- sec.get.condfreq()
#'   head(dt); tail(dt)
sec.get.condfreq <- function(year.quarter = NULL) {
  ...
  dt <- do.call("rbind", lapply(year.quarter, function(yq) { 
    ...
    dtp <- do.call("rbind", lapply(files, function(f) {
      dtpp <- read.csv(unz(temp, paste0("data/", f, ".csv")))

      # Drop the X field, which duplicates Time
      dtpp$X <- NULL

      # Column names to lower case
      colnames(dtpp) <- tolower(colnames(dtpp))
